...
const { data: stats } = await useAsyncData<FarmStat[]>('stats', async () => {
  return baseStats.map((stat) => {
    // Generar valor aleatorio dentro del rango del sensor
    const value =
      stat.minValue + Math.random() * (stat.maxValue - stat.minValue)
    // Generar variación aleatoria
    const variation =
      stat.minVariation +
      Math.random() * (stat.maxVariation - stat.minVariation)
